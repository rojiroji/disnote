...
// 音声ファイルの有声部分を書いたファイルを読み込む
void whisper_init_voice_block_list(char* filename) {
    FILE* fp;
    fopen_s(&fp, filename, "r");

    fprintf(stderr, "voice block file: %s\n", filename);
    ...
    if (fp == NULL) {
        ...
        std::vector<voice_block> vlist(0);

        char buf[2048];
        while ((fgets(buf, sizeof(buf), fp) != NULL)) { // 1行ずつ読み込み
            int start = 0, end = 0;
            char* p = buf;
            for (int i = 0; i < 6; i++) { // ID,分割した音声ファイル名(flac),開始時間(冒頭無音あり),終了時間(末尾無音あり),長さ(無音あり),開始時間(冒頭無音なし),長さ(末尾無音なし)の順
                p = strchr(p, '\t');
                if (p == NULL) {
                    break;
                }
        ...
        whisper_init_voice_block_list(vlist);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (arg == "-s" || arg == "--seed") {
            ...
            if (params.n_threads <= 0) {
                params.n_threads = (int32_t)std::thread::hardware_concurrency();
            }
            ...
            whisper_print_usage(argc, argv, params);
            exit(0);
        } else if (arg == "-vb" || arg == "--voice_blocks") {
            whisper_init_voice_block_list(argv[++i]);
        } else {
    ...
    for (int i = s0; i < n_segments; i++) {
        if (params.no_timestamps) {
            ...
            if (params.print_colors) {
                ...
                printf( "%2d\n%2d\n%s\n", (int)t0 * 10, (int)t1 * 10, text); // XXX 出力改造。t0はミリ秒ではない。t0*10がミリ秒（to_timestamp参照）


                for (int k = 0; k < whisper_full_n_tokens(ctx, i); k++) { // token出力してみる
                    printf("%s", whisper_full_get_token_text(ctx, i, k));
                }
                printf("\n");
                for (int k = 0; k < whisper_full_n_tokens(ctx, i); k++) { // token出力してみる
                    printf("%d,", whisper_full_get_token_id(ctx, i, k));
                }
