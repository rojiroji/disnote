...
std::vector<voice_block> voice_block_list(0);
void whisper_init_voice_block_list(std::vector<voice_block> v) {
    voice_block_list = v;
}
...
int whisper_full(
    ...
    bool is_print_seek = true;
    
    time_t main_tm,tmp_tm;
    time(&main_tm);
    ...
    // seek blockを5個ほど出しておく(デバッグ用)
    if (is_print_seek) {
        fprintf(stderr, "voice blocks(%d)\n", (int)voice_block_list.size());
        for (int i = 0; i < 5 && i < voice_block_list.size(); i++) {
            fprintf(stderr, "voice_block[%i] start=%d,end=%d\n", i, voice_block_list[i].start, voice_block_list[i].end);

        }
    }

    const int threshold = 2;
    while (!(check_count >= threshold)) { // 連続で同じ結果が返ったらNG
        ...
        time(&tmp_tm);
        if (is_print_seek) {
            fprintf(stderr, "seek=%d time=%I64d\n", seek, tmp_tm - main_tm);
        }
        
        // 無音部分にいたら、次の有声部分のstartまで移動する
        while (seek_index < voice_block_list.size() && seek >= voice_block_list[seek_index].end) {
            seek_index++;
            if (is_print_seek) {
                fprintf(stderr, "next seek index. seek_index=%d, start=%d, end=%d\n",
                    seek_index, voice_block_list[seek_index].start, voice_block_list[seek_index].end);
            }
        }
        if (seek_index < voice_block_list.size()) {
            if (seek < voice_block_list[seek_index].start) {
                seek = (voice_block_list[seek_index].start / WHISPER_CHUNK_SIZE) * WHISPER_CHUNK_SIZE; // WHISPER_CHUNK_SIZE で区切る
                if (is_print_seek) {
                    fprintf(stderr, "seek jump. seek=%d\n", seek);
                }
            }
        }
        if (is_print_seek) {
            ...
            for (int i = 0; i < (int)tokens_cur.size(); i++) {
                ...
                if (tokens_cur[i].id > whisper_token_beg(ctx)) {
                    ...
                    if (!text.empty()) {
                        ...
                        if (prev_text.compare(text) == 0) { // 同じ結果だった場合は怪しい
                            //if ((tt1 * 10 - tt0 * 10) % 1000 == 0){ // 認識長が、1000の倍数…と言いたいところだが、毎回1000の倍数にしかならないので微妙
                                //normal = false; // ちょっと考える
                                //text = "*1:" + text;
                                check_count++;
                            //}
                                // ここでcheck_count>nならreturnとすることも考えたが、ループ内の処理で後片付けとかしているかもしれないのでreturnしない
                        }
                        else {
                            check_count = 0;
                        }
